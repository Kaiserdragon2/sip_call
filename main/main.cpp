...
esp_mqtt_client_handle_t mclient = esp_mqtt_client_init(&mqtt_cfg);
    ...
    for (;;)
    {
        ...
        if (!client.is_initialized())
        {
            ...
            client.set_event_handler([&button_input_handler](const SipClientEvent& event) {
                switch (event.event)
                {
                case SipClientEvent::Event::CALL_REQUEST:
                    esp_mqtt_client_publish(mclient,"Klingel","True",0,1,0);
                    break;
